    ...
    def _build_runtime_widget(self, context, root=None):
        if root is not None:
            ...
            intro = QtWidgets.QLabel(
                "Build reaction profiles for the Screen source. Each profile owns rules that say: when this visual pattern appears, NC may respond in this style if background review allows it."
            )
            ...
            note = QtWidgets.QLabel("Each enabled rule should describe a clear screen pattern and the in-character response it should trigger. Open Advanced only when you want tighter control.")
        ...
        def refresh_preview():
            if not self.enabled:
                preview_edit.setPlainText("Disabled. This optional reaction add-on is off in the parent Screen source tab.")
                return
