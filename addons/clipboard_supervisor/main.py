    ...
    return [
        {
            ...
            "name": "Interpreter",
            "style": PERSONA_STYLE_HINTS["Interpreter"],
            "behaviors": [
                {
                    "id": "behavior_any_new_image",
                    "enabled": False,
                    "trigger": "A new clipboard image is present.",
                    "action": "Briefly comment on or interpret the newly copied image for the user.",
...
class Addon(BaseAddon):
    TAB_ID = "clipboard_supervisor_tab"
    CONTRIBUTOR_ID = "nc.clipboard_supervisor.behavior"
    ...
    def _import_legacy_state(self, payload):
        ...
        adult_enabled = bool(payload.get("clipboard_supervisor_watch_adult_content", True))
        horror_enabled = bool(payload.get("clipboard_supervisor_watch_horror_content", True))
        adult_line = str(payload.get("clipboard_supervisor_adult_line") or "").strip() or "HEY there! Back to work. No watching other women than me."
        horror_line = str(payload.get("clipboard_supervisor_horror_line") or "").strip() or "Wow. That looks way too scary for you. Switch to cute kittens before you ruin your sleep."
        behaviors = []
        if adult_enabled:
            behaviors.append(
                {
    ...
    def _add_persona(self, name):
        ...
        print(f"[ClipboardSupervisor] Added persona '{persona_name}'")
        self._publish_state()
    ...
    def _build_runtime_widget(self, context, root=None):
        if root is not None:
            ...
            state_label = ui_child("clipboard_supervisor_state_label", QtWidgets.QLabel)
            add_persona_button = ui_child("btn_clipboard_supervisor_add_persona", QtWidgets.QPushButton)
            rename_persona_button = ui_child("btn_clipboard_supervisor_rename_persona", QtWidgets.QPushButton)
            delete_persona_button = ui_child("btn_clipboard_supervisor_delete_persona", QtWidgets.QPushButton)
            persona_combo = ui_child("clipboard_supervisor_persona_combo", QtWidgets.QComboBox)
            persona_style_edit = ui_child("clipboard_supervisor_persona_style_edit", QtWidgets.QLineEdit)
            ...
            if any(
                ...
                raise RuntimeError("Clipboard Supervisor Designer UI is missing one or more required controls.")
            ...
            template_group = QtWidgets.QGroupBox("Advanced Prompt Rules")
            template_layout = QtWidgets.QVBoxLayout(template_group)
            template_header = QtWidgets.QHBoxLayout()
            template_hint = QtWidgets.QLabel("Advanced: refine how these reaction rules are added to background review.")
            ...
            intro = QtWidgets.QLabel(
                "Build reaction profiles for the Clipboard source. Each profile owns rules that say: when a copied image matches this pattern, NC may respond in this style if background review allows it."
            )
            ...
            note = QtWidgets.QLabel("Each enabled rule should describe a clipboard-image pattern and the in-character response it should trigger. Open Advanced only when you want tighter control.")
            ...
            preview_header = QtWidgets.QLabel("Review Prompt Preview")
        ...
        def refresh_preview():
            if not self.enabled:
                preview_edit.setPlainText("Disabled. This optional reaction add-on is off in the parent Clipboard source tab.")
                return
        ...
        def rebuild_behavior_rows():
            ...
            if not behavior_items:
                empty = QtWidgets.QLabel("No behaviors are configured for this persona yet. Add one to teach the supervisor what kinds of copied images to notice and how to react.")
        ...
        def refresh_from_state():
            ...
            try:
                ...
                state_label.setText(
                    f"The Clipboard Supervisor is active. Persona '{active.get('name')}' currently owns {len(list(active.get('behaviors') or []))} behavior(s)."
                    if self.enabled else
                    "The Clipboard Supervisor is currently inactive. Re-enable it from Clipboard -> Include."
                )
