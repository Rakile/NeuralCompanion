...
FALLBACK_TEMPLATE = """This behavior applies only to webcam input and should behave like a clear policy, not a vague preference.

Use the current webcam image, webcam summary, and retained webcam context together.
...
When one configured behavior clearly matches the webcam scene, decide whether to interrupt in the active persona's voice. Keep interruptions concise, playful, and in-character."""
...
class Addon(BaseAddon):
    ...
    def export_session_state(self):
        return {
            ...
            "webcam_supervisor_prompt_template": str(self.prompt_template or self.recommended_prompt_template or FALLBACK_TEMPLATE),
    ...
    def import_session_state(self, session):
        ...
        try:
            ...
            if "webcam_supervisor_prompt_template" in payload:
                self.prompt_template = self._normalize_prompt_template(payload.get("webcam_supervisor_prompt_template"))
    ...
    def _build_runtime_widget(self, context, root=None):
        if root is not None:
            ...
            state_label = ui_child("webcam_supervisor_state_label", QtWidgets.QLabel)
            add_persona_button = ui_child("btn_webcam_supervisor_add_persona", QtWidgets.QPushButton)
            rename_persona_button = ui_child("btn_webcam_supervisor_rename_persona", QtWidgets.QPushButton)
            delete_persona_button = ui_child("btn_webcam_supervisor_delete_persona", QtWidgets.QPushButton)
            persona_combo = ui_child("webcam_supervisor_persona_combo", QtWidgets.QComboBox)
            persona_style_edit = ui_child("webcam_supervisor_persona_style_edit", QtWidgets.QLineEdit)
            add_behavior_button = ui_child("btn_webcam_supervisor_add_behavior", QtWidgets.QPushButton)
            behaviors_widget = ui_child("webcam_supervisor_behaviors_widget", QtWidgets.QWidget)
            preview_edit = ui_child("webcam_supervisor_preview_edit", QtWidgets.QPlainTextEdit)
            preview_label = ui_child("webcam_supervisor_preview_label", QtWidgets.QLabel)
            if any(
                item is None
                for item in (
                    state_label,
                ...
                raise RuntimeError("Webcam Supervisor Designer UI is missing one or more required controls.")
            ...
            intro = QtWidgets.QLabel(
                "Build reaction profiles for the Webcam source. Each profile owns rules that say: when this posture, expression, or attention cue appears, NC may respond in this style if background review allows it."
            )
            ...
            note = QtWidgets.QLabel("Each enabled rule should describe a clear webcam pattern and the in-character response it should trigger. Open Advanced only when you want tighter control.")
        ...
        def refresh_preview():
            if not self.enabled:
                preview_edit.setPlainText("Disabled. This optional reaction add-on is off in the parent Webcam source tab.")
                return
        ...
        def refresh_from_state():
            ...
            try:
                ...
                state_label.setText(
                    ...
                    "The Webcam Supervisor is currently inactive. Re-enable it from Webcam -> Include."
                )
