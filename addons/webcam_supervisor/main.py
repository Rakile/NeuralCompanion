    ...
    def _build_runtime_widget(self, context, root=None):
        if root is not None:
            ...
            intro = QtWidgets.QLabel(
                "Build one or more supervisor personas for the Webcam source. Each persona can own several behaviors, where every behavior pairs a visual trigger with the action the hidden LLM should take."
            )
            ...
            note = QtWidgets.QLabel("Each enabled behavior should describe a clear webcam pattern and the in-character interruption it should trigger. Open Advanced only when you want tighter control.")
        ...
        def refresh_preview():
            if not self.enabled:
                preview_edit.setPlainText("Disabled. This child behavior is currently excluded by its parent Webcam source tab.")
                return
