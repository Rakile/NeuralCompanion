...
def _load_controller_class():
    ...
    module_name = "nc_addon_chatterbox_multilingual_tts_controller"
    spec = importlib.util.spec_from_file_location(module_name, controller_path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Could not load Chatterbox Multilingual controller from {controller_path}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module.ChatterboxMultilingualTTSController


def _load_service_class():
    service_path = Path(__file__).with_name("service.py")
    module_name = "nc_addon_chatterbox_multilingual_tts_service"
    spec = importlib.util.spec_from_file_location(module_name, service_path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Could not load Chatterbox Multilingual service from {service_path}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module.ChatterboxMultilingualTTSService


class Addon(BaseAddon):
    SERVICE_NAME = "chatterbox_multilingual"
    TAB_ID = "chatterbox_multilingual_tts_tab"
    ...
    def initialize(self, context):
        ...
        context.logger.info("Chatterbox Multilingual TTS addon initialized.")
    ...
    def invoke_capability(self, capability, payload=None):
        ...
        from addons.chatterbox_multilingual_tts import real_ui_bridge
